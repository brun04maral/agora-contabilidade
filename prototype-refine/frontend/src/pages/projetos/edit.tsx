...
export const ProjetoEdit: React.FC = () => {
  ...
  const { selectProps: clienteSelectProps } = useSelect({
    ...
    optionLabel: "nome",
    optionValue: "id",
    defaultValue: queryResult?.data?.data?.cliente_id,
  });
  ...
  return (
    <Edit saveButtonProps={saveButtonProps}>
      <Form {...formProps} layout="vertical">
        ...
        <Form.Item label="Valor Total (€)" name="valor_total" rules={[{ required: true }]}>
          <InputNumber style={{ width: "100%" }} min={0} precision={2} />
        </Form.Item>

        <Form.Item label="Prémio Bruno (€)" name="premio_bruno">
          <InputNumber style={{ width: "100%" }} min={0} precision={2} />
        </Form.Item>

        <Form.Item label="Prémio Rafael (€)" name="premio_rafael">
          <InputNumber style={{ width: "100%" }} min={0} precision={2} />
        </Form.Item>
