...
Q: Como faço backup?
A: Automático: script cron diário (ver MANUTENCAO.md)
   Manual: pg_dump (ver MANUTENCAO.md seção 1.3)
   Uploads: tar -czf dos ficheiros data/uploads/
...
A: Via Prisma Studio ou SQL:
   1. Gerar hash nova password: https://bcrypt-generator.com/
   2. UPDATE account SET password = '$2a$10$...' WHERE account_id = 'email';
...
A: INs (entradas):
   - Projetos pessoais RECEBIDOS (projectCode = PESSOAL_X, category = RECEBIDO)
   - Prémios de projetos empresa (extra.premio_bruno / premio_rafael)
   
   OUTs (saídas):
   - Despesas fixas PAGAS ÷ 2 (repartidas)
   - Boletins emitidos (categoryCode = BOLETIM, socio = X)
   - Despesas pessoais PAGAS (categoryCode = DESPESA_PESSOAL_X)
...
A: 1. Abrir Prisma Studio: npx prisma studio
   ...
   4. Ver FAQ "Troubleshooting" abaixo
...
A: Método linear:
   Valor Actual = Valor Compra × (1 - (Idade / Vida Ütil))
...
A: 1. Verificar API key válida
   ...
   3. Ver logs erro (docker-compose logs)
   4. Retry manual
...
A: PostgreSQL não acessível.
   1. Verificar PostgreSQL a correr: systemctl status postgresql
   2. Testar conexão manual: psql $DATABASE_URL
...
A: 1. Ver logs: docker-compose logs -f app
   2. Abrir DevTools browser (F12) → Console
   3. Verificar .env tem todas as variáveis
   4. Verificar migrations aplicadas: npx prisma migrate status

Q: Upload ficheiros não funciona
A: 1. Verificar directorio existe: ls -la ./data/uploads
   2. Verificar permissões: chmod 755 ./data/uploads
...
A: 1. Verificar email correcto
   2. Reset password (ver FAQ acima)
   3. Verificar Better Auth configurado: echo $BETTER_AUTH_SECRET
   4. Ver logs auth: docker-compose logs app | grep auth

Q: Backup automático não está a correr
A: 1. Verificar cron activo: systemctl status cron
   2. Ver crontab: crontab -l
   3. Testar script manual: ./scripts/backup-db.sh
   4. Ver logs: cat /var/log/agora-backup.log

Q: App lenta / queries demoram
A: 1. Verificar indexes database (ver MANUTENCAO.md)
   2. Enable query logging: DEBUG=prisma:query no .env
   3. Identificar queries lentas nos logs
   4. Adicionar indexes conforme necessário
   5. Considerar VACUUM PostgreSQL: VACUUM ANALYZE;
...
A: 1. Parar imediatamente
   2. Limpar database TaxHacker: npx prisma migrate reset
   3. Corrigir scripts
   4. Tentar novamente
...
A: Next.js 15 App Router:
   1. Criar pasta app/(app)/nova-rota/
   2. Criar page.tsx:
...
A: 1. Editar prisma/schema.prisma
   2. Criar migration: npx prisma migrate dev --name add_new_model
   3. Gerar client: npx prisma generate
   4. Usar no código: prisma.newModel.create(...)

Q: Como testar antes de deploy?
A: 1. Rodar testes: npm run test (quando implementados)
   2. Build produção: npm run build
...
A: Docker (recomendado):
   ...
   5. Verificar: curl http://localhost:7331/api/health
...
A: Sim, se seguiste MANUTENCAO.md.
   Verificar:
   - Cron a correr: crontab -l
   - Backups recentes: ls -lh /var/backups/agora-contabilidade/
...
A: 1. Health check endpoint: /api/health
   ...
   3. Métricas PostgreSQL: verificar tamanho DB
   4. Espaço disco: df -h
...
A: 1. Backup primeiro!
   ...
   3. npm install (se package.json mudou)
   4. npx prisma migrate deploy (se schema mudou)
   5. npm run build
   6. docker-compose restart (ou rebuild se Dockerfile mudou)
   7. Testar
...
A: Sim, mas NÃO RECOMENDADO (contabilidade = histórico importante).
   ...
   3. DELETE FROM transactions WHERE "issuedAt" < '2020-01-01';
...
A: 1. Este FAQ (primeira linha!)
   ...
   3. Logs sistema (docker-compose logs)
...
A: 1. Adicionar upstream: git remote add upstream https://github.com/vas3k/TaxHacker.git
   2. Fetch: git fetch upstream
   3. Review changes: git log upstream/main
   4. Cherry-pick features interessantes: git cherry-pick <commit>
...
A: Sim!
   ...
   - Database completa: pg_dump
   - Ficheiros: copiar data/uploads/
...
Self-hosted: Hospedar em infraestrutura própria (vs cloud SaaS)
